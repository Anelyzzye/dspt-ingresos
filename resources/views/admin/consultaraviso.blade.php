...
<form  method="GET" action="{{ route('avisos.index') }}" >
    <h4 class="text-white mb-0">Publicaciones/avisos </h4>
      	<div class="input-group-prepend">
      		<div class="input-group-text">
            <span class="fa fa-search">
            </span>
            </div>
        <input type="text" name="titulo" class="bg8 s-text6" placeholder="Ingrese el nombre">
    	</div>
    	<div><br></div>
  <input type="submit" value="Visualizar registros" class="tag-item size3 m-text2 hov2 trans-0-4"> 
		...
		<div><br></div>
        <div class="col">
          <div class="card bg-default shadow"><h3 class="text-white fs-33 mb-0">Avisos publicados</h3>
                    ...
                    <th scope="row">
                      <div class="media align-items-center">
                        ...
                        <div class="media-body">
                          <span class="mb-0 text-lg">{{ $avi->aviso_titulo }}  </span>
                        </div>
                      </div>
                    </th>
                   
                    <td>
                      <a href="{{ route('avisos.edit', $avi->id) }}">
                    	<input type="submit" value="Editar" class=" tag-item size2 m-text3 hov3 trans-0-4"></a>
                    </td>

                     <td>
                      <form action=" {{ route('avisos.delete', $avi->id)}}" method="POST">
                          @method("DELETE")
                          @csrf
